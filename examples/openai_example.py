"""
SMOLFuzz — OpenAI backend example.

Compatible with any OpenAI-compatible endpoint:
    OpenAI, Together AI, Fireworks AI, vLLM, etc.
...
Run from the parent directory of this repo:
    python3 -m smolfuzz.examples.openai_example

To use a compatible endpoint (e.g. Together AI):
    import openai
    client_raw = openai.OpenAI(
        base_url="https://api.together.xyz/v1",
        api_key=os.environ["TOGETHER_API_KEY"],
    )
    # Pass the raw client via monkeypatch or extend OpenAIClient.
"""
...
OUTPUT_DIR  = Path("results/openai_run")
N_MODELS    = 10
API_SET     = 30
BUDGET      = 60
